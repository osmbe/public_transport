...
leaveCityNameAnywayRE = re.compile(
    r"(?ix)((^|\s)((Noord|Zuid)?station|dorp|markt|kerk|centrum|Veldekensstraat)(\s|$))")  # case insensitive search for terms surrounded by whitespace
...
common_abbreviations = [
    ...
    (500, 0, r"(?ui)\bP\.*\s*Coomans", "Pieter Coomans"),
...
def convert_stop_name(identifier, zone=''):
    assert isinstance(identifier, str)
    name = identifier
    for order, final, short, long in common_abbreviations:
        if short == long and short == name:
            if final:
                break
            else:
                continue
        ...
        if original_name != name and final:
            break
    return omit_larger_city_names(name, zone).strip()


def omit_larger_city_names(name, zone=''):
    if (name and
            not (leaveCityNameAnywayRE.search(name)) and
            not (zone in ['79', '80', '61', '68']) and
            not ("Sas van Gent" in name)):
        # zone 79,80: Vorst bij Veerle, zone 61,68: Deurne bij Diest, zone 31: Sas van Gent
        for city, replacement in city_names_to_omit:
            if city in name:
...
def escape_url(name):
    return escape_xml(name).replace(' ', '%20')
